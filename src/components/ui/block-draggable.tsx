    ...
    <div
      ...
      onMouseDown={(e) => {
        ...
        if (!selectionNodes.some(([node]) => node.id === element.id)) {
          selectionNodes = [[element, editor.api.findPath(element) as Path]]
        }
      ...
      onMouseEnter={() => {
        ...
        if (!selectedBlocks.some(([node]) => node.id === element.id)) {
          selectedBlocks = [[element, editor.api.findPath(element) as Path]]
        }
...
const createDragPreviewElements = (editor: PlateEditor, blocks: TElement[]): HTMLElement[] => {
  ...
  const resolveElement = (node: TElement, index: number) => {
    const domNode = editor.api.toDOMNode(node) as HTMLElement
    ...
    if (lastDomNode) {
      const lastDomNodeRect = (
        (editor.api.toDOMNode(lastDomNode) as HTMLElement).parentElement as HTMLElement
      ).getBoundingClientRect()

      const domNodeRect = (domNode.parentElement as HTMLElement).getBoundingClientRect()
  ...
  blocks.map((node, index) => resolveElement(node, index))
...
const calculatePreviewTop = (
  ...
  const child = editor.api.toDOMNode(element) as HTMLElement
  const editable = editor.api.toDOMNode(editor) as HTMLElement
  const firstSelectedChild = blocks[0]

  const firstDomNode = editor.api.toDOMNode(firstSelectedChild) as HTMLElement
  ...
  const currentMarginTopString = window.getComputedStyle(child as HTMLElement).marginTop
...
const calcDragButtonTop = (editor: PlateEditor, element: TElement): number => {
  ...
  const currentMarginTopString = child ? window.getComputedStyle(child as HTMLElement)?.marginTop : ''
  const currentMarginTop = Number(currentMarginTopString.replace('px', ''))
