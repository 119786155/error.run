      ...
      <div className="flex flex-col p-4">
            ...
            {suggestion.type === 'remove' &&
              suggestionText2Array(suggestion.text).map((text) => (
                <div key={text} className="flex items-center gap-2">
                  ...
                  <span key={text} className="text-sm">
                    {text}
                  </span>
                </div>
              ))}
              ...
              <div className="flex flex-col gap-2">
                {suggestionText2Array(suggestion.newText).map((text) => (
                  <React.Fragment key={text}>
                    <div key={text} className="flex items-start gap-2 text-brand/80">
        ...
        {hovering && (
          <div className="absolute top-4 right-4 flex gap-2">
            <Button variant="ghost" className="size-6 p-1 text-muted-foreground" onClick={() => accept(suggestion)}>
              <CheckIcon className="size-4" />
            </Button>

            <Button variant="ghost" className="size-6 p-1 text-muted-foreground" onClick={() => reject(suggestion)}>
