    ...
    <div className="flex h-6 items-center rounded-md bg-muted/60 p-0">
      ...
      <Popover open={isFocused} modal={false}>
        <PopoverTrigger asChild>
          <input
            className={cn('h-full w-6 shrink-0 bg-transparent px-1 text-center text-sm hover:bg-muted')}
            value={displayValue}
            onBlur={() => {
              setIsFocused(false)
              handleInputChange()
        ...
        <PopoverContent className="w-6 px-px py-1" onOpenAutoFocus={(e) => e.preventDefault()}>
          {FONT_SIZES.map((size) => (
            <button
              key={size}
              className={cn(
