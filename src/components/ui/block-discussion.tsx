...
export const BlockDiscussion: RenderNodeWrapper<AnyPluginConfig> = (props) => {
  ...
  const suggestionNodes = [...editor.getApi(SuggestionPlugin).suggestion.nodes({ at: blockPath })].filter(
...
const BlockCommentContent = ({
  ...
  const commentingBlock = usePluginOption(BaseCommentKit, 'commentingBlock')
  const activeCommentId = usePluginOption(BaseCommentKit, 'activeId')
    ...
    if (activeCommentId) {
      if (activeCommentId === getDraftCommentKey()) {
        ...
        activeNode = commentNodes.find(
          ([node]) => editor.getApi(BaseCommentKit).comment.nodeId(node) === activeCommentId,
        )
      }
    }
...
const useResolvedDiscussion = (commentNodes: NodeEntry<TCommentText>[], blockPath: Path) => {
  const { api, getOption, setOption } = useEditorPlugin(BaseCommentKit)
