...
function RemoteCursorOverlayContent() {
  const containerRef: HTMLDivElement | null = useEditorContainerRef()
  ...
  return (
    <>
      {selectionRects.map((position) => (
        <div key={Math.random()} className="pointer-events-none absolute" style={{ ...selectionStyle, ...position }} />
      ))}
...
function Caret({ caretPosition, data }: Pick<CursorOverlayData<CursorData>, 'caretPosition' | 'data'>) {
