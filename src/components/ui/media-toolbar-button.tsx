...
> = {
  [KEYS.audio]: {
    ...
    title: getContent('editor.upload.audio'),
    tooltip: getContent('editor.upload.audio'),
  },
  [KEYS.file]: {
    accept: ['*'],
    icon: <FileUpIcon className="size-4" />,
    title: getContent('editor.upload.file'),
    tooltip: getContent('editor.upload.file'),
  },
  [KEYS.img]: {
    accept: ['image/*'],
    icon: <ImageIcon className="size-4" />,
    title: getContent('editor.upload.image'),
    tooltip: getContent('editor.upload.image'),
  },
  [KEYS.video]: {
    accept: ['video/*'],
    icon: <FilmIcon className="size-4" />,
    title: getContent('editor.upload.video'),
    tooltip: getContent('editor.upload.video'),
  },
}
      ...
      >
        ...
        <DropdownMenu open={open} onOpenChange={setOpen} modal={false} {...props}>
          <DropdownMenuTrigger asChild>
            <ToolbarSplitButtonSecondary onClick={(e) => e.stopPropagation()} />
          </DropdownMenuTrigger>
          ...
          <DropdownMenuContent onClick={(e) => e.stopPropagation()} align="start" alignOffset={-32}>
            <DropdownMenuGroup>
              <DropdownMenuItem onSelect={() => openFilePicker()}>
                {currentConfig.icon}
                {getContent('editor.upload.local')}
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => setDialogOpen(true)}>
                <LinkIcon />
                {getContent('editor.upload.url')}
              </DropdownMenuItem>
            </DropdownMenuGroup>
          </DropdownMenuContent>
        </DropdownMenu>
