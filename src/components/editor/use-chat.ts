...
export type ChatMessage = UIMessage<Record<string, string>, MessageDataPart>
        ...
        if (!res.ok) {
          ...
          try {
            const content = JSON.parse(init?.body as string)
              .messages.at(-1)
              .parts.find((p: Record<string, string>) => p.type === 'text')?.text
    ...
    onData(data) {
      ...
      if (data.type === 'data-comment' && data.data) {
        ...
        const newDiscussion = {
          ...
          comments: [newComment],
          createdAt: new Date(),
          documentContent: deserializeMd(editor, aiComment?.content as string)
      ...
      if (sample === 'comment') {
        ...
        for (let i = 0; i < blocks.length; i++) {
          ...
          for (const chunk of block) {
            ...
            controller.enqueue(
              encoder.encode(`data: {"type":"text-delta","id":"${messageId}","delta":"${escapedText}"}\n\n`),
            )
          }

          // Add double newline after each block except the last one
          if (i < blocks.length - 1) {
            controller.enqueue(encoder.encode(`data: {"type":"text-delta","id":"${messageId}","delta":"\\n\\n"}\n\n`))
          }
        }
